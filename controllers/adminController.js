    ...
    try {
        productUpload.fields([
            ...
            try {
                ...
                if (req.files.mainImage) {
                    mainImagePath = '/uploads/products/' + req.files.mainImage[0].filename;
                    
                    // Xóa ảnh cũ
                    if (product.mainImage) {
                        const oldMainImagePath = path.join('public', product.mainImage);
                        if (fs.existsSync(oldMainImagePath)) {
                            fs.unlink(oldMainImagePath, err => {
                                if (err) console.error('Lỗi xóa ảnh chính cũ:', err);
                            });
                        }
                    }
                }
                
                // Xử lý ảnh phụ đã xóa
                let currentImages = product.images || [];
                if (deletedImages) {
                    try {
                        const deletedImagesArray = JSON.parse(deletedImages);
                        // Xóa file ảnh đã xóa
                        deletedImagesArray.forEach(imgUrl => {
                            // Cập nhật danh sách ảnh hiện tại
                            currentImages = currentImages.filter(img => img !== imgUrl);
                            
                            // Xóa file
                            const imgPath = path.join('public', imgUrl);
                            if (fs.existsSync(imgPath)) {
                                fs.unlink(imgPath, err => {
                                    if (err) console.error('Lỗi xóa ảnh phụ:', err);
                                });
                            }
                        });
                    } catch (e) {
                        console.error('Lỗi xử lý danh sách ảnh đã xóa:', e);
                    }
                }
                ...
                if (req.files.productImages) {
                    ...
                    // Giới hạn số lượng ảnh phụ là 5
                ...
                product.active = active === 'on' || active === true;
...
exports.deleteProduct = async (req, res) => {
    try {
        ...
        // Xóa ảnh chính
        if (product.mainImage) {
            const mainImagePath = path.join('public', product.mainImage);
            if (fs.existsSync(mainImagePath)) {
                fs.unlink(mainImagePath, err => {
                    if (err) console.error('Lỗi xóa ảnh chính:', err);
                });
            }
        }
...
exports.getCoupons = async (req, res) => {
    try {
        ...
        console.error('Lỗi lấy danh sách mã khuyến mãi:', error);
        res.status(500).send('Lỗi server');
    }
};

// Hiển thị form thêm mã khuyến mãi
exports.getAddCouponForm = (req, res) => {
...
exports.addCoupon = async (req, res) => {
    try {
        ...
        if (couponExists) {
            req.session.couponData = req.body;
        ...
        console.error('Lỗi thêm mã khuyến mãi:', error);
...
exports.getEditCouponForm = async (req, res) => {
    try {
        ...
        if (!coupon) {
            return res.redirect('/admin/coupons?error=not-found');
        }
        ...
        console.error('Lỗi lấy thông tin mã khuyến mãi:', error);
        res.redirect('/admin/coupons?error=not-found');
    }
};
...
exports.updateCoupon = async (req, res) => {
    try {
        ...
        // Kiểm tra mã khuyến mãi đã tồn tại chưa (nếu thay đổi)
        if (code.toUpperCase() !== coupon.code) {
            ...
            if (couponExists) {
                return res.render('admin/coupon/edit', {
                    title: 'Chỉnh sửa mã khuyến mãi',
                    error: 'Mã khuyến mãi đã tồn tại',
                    coupon: { _id: req.params.id, ...req.body },
                    user: req.session.user
                });
            }
        }
...
exports.getReviews = async (req, res) => {
    try {
        ...
        res.status(500).render('page-error-404', {
            message: 'Đã xảy ra lỗi khi tải trang quản lý đánh giá',
            user: req.session.user
        });
    }
};
...
exports.approveReview = async (req, res) => {
    try {
        ...
        res.status(200).json({
            success: true,
            message: 'Đã phê duyệt đánh giá thành công',
            review
        });
    } catch (error) {
...
exports.getOrderDetail = async (req, res) => {
    try {
        ...
        if (!order) {
            return res.redirect('/admin/orders?error=not-found');
        }

        res.render('admin/order/detail', { order, title: 'Chi tiết đơn hàng' });
    } catch (error) {
        console.error('Lỗi khi lấy chi tiết đơn hàng:', error);
        res.status(500).send('Đã xảy ra lỗi khi lấy chi tiết đơn hàng');
    }
};
...
exports.getEditOrder = async (req, res) => {
    try {
        ...
        res.render('admin/order/edit', { order, error: null, title: 'Cập nhật đơn hàng' });
    } catch (error) {
        console.error('Lỗi khi lấy thông tin đơn hàng để chỉnh sửa:', error);
...
exports.updateOrder = async (req, res) => {
    try {
        ...
        if (!order) {
            return res.render('admin/order/edit', {
                order: { _id: orderId },
        ...
        // Nếu chuyển trạng thái sang "Đã thanh toán" và phương thức là COD
        ...
        res.status(500).send('Đã xảy ra lỗi khi cập nhật đơn hàng');
    }
};
